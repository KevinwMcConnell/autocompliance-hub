...
export default function Tasks() {
  ...
  const { demoLoaded, tasks, loadDemoData, updateTask } = useDemoData();
  ...
  const handleComplete = (task: DemoTask) => {
  ...
  const TaskCard = ({ task, onComplete }: { task: DemoTask; onComplete: (task: DemoTask) => void }) => {
    const status = getDueStatus(task.daysUntilDue);

    return (
      <Card className={task.status === "overdue" ? "border-status-overdue/50" : ""}>
